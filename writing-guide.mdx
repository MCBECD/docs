...
|------|------|------|
| `title` | ✅ | 文档标题 |
| `order` | ✅ | 排序权重（数字越小越靠前，建议间隔 10） |
| `description` | ✅ | 简短描述（用于 SEO 和列表展示） |
| `author` | ✅ | 作者名称 |
| `updatedAt` | ✅ | 更新日期，格式 `YYYY-MM-DD` |
| `category` | 推荐 | 分类：`intro` / `basics` / `commands` |
| `tags` | 可选 | 标签数组，用于筛选 |
...
> 如果不指定语言，默认使用 `mcfunction` 高亮。也可以使用其他 Shiki 支持的语言，如 `json`、`javascript`、`bash` 等。
...
├── give-diamonds/
│   ├── index.mdx      # 文档正文
│   └── meta.json      # 额外元数据（会覆盖 frontmatter）
```

`meta.json` 中指定的字段会覆盖 frontmatter 中的同名字段。
