...
- 通配符 `*` 可指定所有已追踪玩家
- 显示名称支持格式化代码（§），如 `§e金币`
- 结合 `/execute` 命令（特别是 `/execute if score`）可实现复杂的条件逻辑判断，详见 /execute 文档
- 配合 `/tag` 命令可以为实体添加标记，再通过计分板实现更精细的数据分类和逻辑控制，详见 /tag 文档
- 常与 `/effect` 命令结合使用，例如根据计分板分数决定是否给予玩家特定效果，详见 /effect 文档
