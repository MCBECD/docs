---
author: "官方•Dingding OvO"
updatedAt: "2026-08-12"
title: "/tag  实体标签"
category: commands
description: "添加、移除或列出实体标签，用于标记与定位"
tags: ["标签", "实体", "命令方块", "自动化", "数据追踪", "OP1", "批量"]
---

添加、移除或列出实体上的标签。标签用于标记实体，方便后续定位和逻辑判断。

### 语法

`/tag <实体> add <名称>`

`/tag <实体> remove <名称>`

`/tag <实体> list`

### 参数

- `<实体>` — 目标实体
- `<名称>` — 标签名称（最多 32 个字符，字母数字、下划线和连字符）
- `add` — 为实体添加标签
- `remove` — 从实体移除标签
- `list` — 列出实体上的所有标签

### 示例

**给自己添加"admin"标签：**
```mcfunction
/tag @s add admin
```

为执行者添加 `admin` 标签，后续可通过 `@a[tag=admin]` 选中所有管理员玩家。

**给所有僵尸添加"undead"标签：**
```mcfunction
/tag @e[type=zombie] add undead
```

标记所有僵尸为亡灵类，在 [`/effect`](../commands/effect/) 中可批量给予或不给予特定效果。

**移除标签：**
```mcfunction
/tag @s remove admin
```

从执行者身上移除 `admin` 标签。

**列出自己的所有标签：**
```mcfunction
/tag @s list
```

在聊天栏显示执行者当前拥有的所有标签名称。

**清除所有带特定标签的实体：**
```mcfunction
/kill @e[tag=cleanup]
```

标签常与 [`/kill`](../commands/kill/) 配合，批量移除标记为 `cleanup` 的临时实体。

### 标签与其他命令配合使用

标签通过目标选择器与其他命令结合使用时最为强大：

**给所有带"winner"标签的实体钻石：**
```mcfunction
/give @e[tag=winner,type=player] diamond 64
```

使用 [`/give`](../commands/give/) 奖励带有胜利标签的玩家。

**传送所有"team_red"玩家：**
```mcfunction
/tp @a[tag=team_red] 0 64 0
```

配合 [`/tp`](../commands/tp/) 批量传送同队玩家到指定位置。

**给所有"buffed"玩家添加效果：**
```mcfunction
/effect @a[tag=buffed] strength 30 2 true
```

配合 [`/effect`](../commands/effect/) 为标记玩家批量添加状态效果。

### 基岩版注意

- 标签是持久的，实体重新加载（存档重进）后仍然存在
- 标签区分大小写 — `Admin` 和 `admin` 是不同的标签
- 一个实体可以同时拥有多个标签
- 标签广泛用于小游戏队伍管理和逻辑判断
- 在命令系统中使用标签而非名称进行可靠的实体定位
- 需要 OP 等级 1
